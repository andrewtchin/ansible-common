...
echo "Run ubuntu-desktop playbook"
ansible-playbook -vvv playbooks/ubuntu-desktop.yml --ask-become-pass -c local --extra-vars=@vars/ubuntu.json -i inventory

echo "Running developer playbook"
ansible-playbook -vvv playbooks/developer.yml --ask-become-pass -c local --extra-vars=@vars/ubuntu.json -i inventory
